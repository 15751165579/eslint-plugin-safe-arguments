 ...
 * 判断当前表达式是否具备至少两个参数。
 * 
 * @param {Obejct} memberExpression 表达式成员对象
 * @param {Object} args 参数对象 
 */
const isSafeArguments = (memberExpression, args) => {
  if (isCall(memberExpression) && args.length < 3) {
    return false;
  }

  if (isApply(memberExpression) && !(args[1] && args[1].type === 'ArrayExpression' && args[1].elements.length > 1)) {
    return false;
  }
...
module.exports = {
  create: function(context) {
    ...
    const checkMathMaxOrMinSafe = node => {
      ...
      if (!isSafeArguments(memberExpression, args)) {
        report(context, node);
      }
    }
    return {
